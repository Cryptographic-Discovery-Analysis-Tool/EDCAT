...
> **Vendoring note (added 2026-09-20).** This is the master planning document the user supplied
> from `Downloads/PRAMANA_FINAL_2_Implementation_Plan_and_Rating.md`, copied here verbatim so it
> lives in the repository rather than only on one machine. It is a **planning/rating document**,
> not one of the four canonical, READ-ONLY architecture sources CLAUDE.md lists — `docs/build-plan.md`
> remains "the only phase numbering" per CLAUDE.md, and its "Ledger phases 1–8" table is the
> up-to-date record of what in §7.2 below is actually done. §5 of this document (the frozen
> exposure-ledger spec) was already vendored separately as `docs/architecture/Pramana_Ledger_Spec.md`
> and is hashed in `docs/architecture/HASHES.lock`; this file is **not** added to that lock because
> it is not itself cited by `rule_id` in code the way the ledger spec is. Where this document's
> phase list (§7) and `docs/build-plan.md`'s progress tables disagree on what is done, treat
> `docs/build-plan.md` as current — it is updated every session; this file is a snapshot from
> 18 Sep 2026.
