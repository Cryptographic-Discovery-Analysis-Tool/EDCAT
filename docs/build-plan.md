...
| `tls-endpoint --live` wired into the CLI (2026-09-26, DEV-013 follow-up) | **done, openssl-only** | `cli.py::_build_tls` no longer refuses `--live`: `adapters/tls/adapter.py::live_tls_probe_runner` composes DEV-004's full-offer/classical-only `openssl s_client` probes with DEV-013's per-group probes into one factory, gated on one `openssl version` check (`--openssl-bin`, else `ECDAT_OPENSSL_BIN`, else `PATH`; below 3.5.0 or missing, every probe is skipped and `hybrid_kex_supported` reports UNKNOWN with a visibility note naming what was found). sslyze is deliberately **not** wired live — its own live wiring is the separate AGPL-3.0 separate-process boundary (spec §3), still not attempted — so a live scan's sslyze-sourced fields (`supported_curves`, `der_sha256`, `leaf_subject`, ...) stay absent, and `coverage.skipped` says `"sslyze: not run for this target"` on every live run. 6 new CLI-level tests in `tests/unit/test_cli.py` (live wiring produces hybrid findings; too-old/missing openssl reports UNKNOWN with a visibility note; `--openssl-bin` threads through the actual argv), all driven through `main()` with `subprocess.run` monkeypatched to replay real recordings from `tests/fixtures/recorded/openssl/3.5.4/hybrid_groups_probe/` — no network in tests. **Manually verified live** (not a test, per CLAUDE.md's replay-is-for-tests-only rule): a throwaway self-signed cert + `openssl s_server` (OpenSSL 3.5.4, `-groups X25519MLKEM768:X25519`) on `127.0.0.1:15443` produced `negotiated_group=X25519MLKEM768`, `hybrid_kex_supported=True`, `classical_still_accepted=True`, all `KNOWN`; the same command against `cloudflare.com:443` over the real network produced the same `negotiated_group=X25519MLKEM768` and additionally accepted `secp256r1` individually (3/6 groups accepted) — a real, independently-observed public endpoint already negotiating ML-KEM hybrid key exchange. |
